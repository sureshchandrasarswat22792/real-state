...
export default function UpdateListing() {
    ...
    const { id: getListingId } = useParams();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setLoading(true);
            setError(false);
            const res = await fetch(`/api/listing/update/${getListingId}`, {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        const fetchListings = async () => {
            try {
            const res = await fetch(`/api/listing/get/${getListingId}`);
            const data = await res.json();
            setFormData(data);
            } catch (error) {
    ...
    return (
        <div className='my-10 w-[65%] mx-auto'>
            <main>
                <h1 className='font-bold text-center mb-7 text-3xl'>Update a Listing</h1>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className='text-center mt-4'>
                        <button type='submit' disabled={loading || uploading} className='bg-slate-700 text-white border rounded px-8 py-3 w-full'>{loading ? 'Updating...' : 'Update listing'}</button>
